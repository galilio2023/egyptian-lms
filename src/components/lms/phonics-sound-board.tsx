...
interface PhonicsItem {
  ...
  sound: string; // e.g. "Sh"
  label: string; // e.g. "ش"
  exampleWord: string; // e.g. "Ship"
  translation: string; // e.g. "سفينة"
  emoji: string;
  category: "vowels" | "digraphs" | "alphabet";
}

const PHONICS_DATA: PhonicsItem[] = [
  // Alphabet & Short Vowels
...
export function PhonicsSoundBoard() {
  ...
  const speakText = useCallback((word: string, soundId: string) => {
    ...
    utterance.pitch = 1.15; // friendly, upbeat pitch for kids
        ...
        {filteredItems.map((item) => {
          ...
          return (
            <button
              ...
              className={`p-3.5 rounded-2xl border-2 transition-all duration-300 text-right flex flex-col justify-between gap-3 relative overflow-hidden group hover:scale-[1.03] hover:-translate-y-1 ${
                isCurrent

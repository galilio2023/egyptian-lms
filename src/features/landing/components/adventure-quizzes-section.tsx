...
export function AdventureQuizzesSection() {
  return (
    <section id="adventure_quizzes" className="py-20 bg-gradient-to-b from-white/70 via-purple-50/30 to-white/80 border-t border-purple-100 relative overflow-hidden">
      ...
      <div className="absolute top-1/4 -start-24 w-96 h-96 bg-purple-300/15 rounded-full blur-3xl pointer-events-none" aria-hidden="true" />
      <div className="absolute bottom-1/4 -end-24 w-96 h-96 bg-amber-300/15 rounded-full blur-3xl pointer-events-none" aria-hidden="true" />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="text-center space-y-3 mb-14">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-purple-100/90 border border-purple-200 text-purple-900 text-xs font-bold shadow-xs">
            <Sparkles className="w-4 h-4 text-amber-500 animate-[spin_4s_linear_infinite]" aria-hidden="true" />
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {INITIAL_ADVENTURE_QUIZZES.map((adv) => (
            ...
            >
              ...
              <div className="absolute -bottom-4 -start-4 opacity-10 group-hover:opacity-20 transition-opacity w-28 h-28 pointer-events-none" aria-hidden="true">
                {adv.theme === "zoo" && <ToyDinoDinoSvg className="w-full h-full" aria-hidden="true" />}
                {adv.theme === "spiderman" && <ToySpiderHeroSvg className="w-full h-full" aria-hidden="true" />}
                {adv.theme === "fruits" && <ToyMagmaAppleSvg className="w-full h-full" aria-hidden="true" />}
                {adv.theme === "numbers" && <ToyStackingBlocksSvg className="w-full h-full" aria-hidden="true" />}
              </div>
              ...
              <div className="flex items-center justify-between gap-2 mb-4">
                ...
                <span className="inline-flex items-center gap-1 text-[11px] font-extrabold text-amber-700 bg-amber-100/90 px-2.5 py-0.5 rounded-full border border-amber-200">
                  <XpGemSvg className="w-3.5 h-3.5" aria-hidden="true" />
                  <span>+{adv.xpReward} XP</span>
                </span>
              </div>

              {/* Central Mascot Icon */}
              <div className="w-20 h-20 mx-auto rounded-3xl bg-white shadow-md p-3 flex items-center justify-center group-hover:scale-110 transition-transform duration-300 border border-white/80 my-2" aria-hidden="true">
                {adv.theme === "zoo" && <ToyDinoDinoSvg className="w-full h-full drop-shadow" aria-hidden="true" />}
                {adv.theme === "spiderman" && <ToySpiderHeroSvg className="w-full h-full drop-shadow" aria-hidden="true" />}
                {adv.theme === "fruits" && <ToyMagmaAppleSvg className="w-full h-full drop-shadow" aria-hidden="true" />}
                {adv.theme === "numbers" && <ToyStackingBlocksSvg className="w-full h-full drop-shadow" aria-hidden="true" />}
              </div>
        ...
        <div className="mt-12 p-4 rounded-2xl bg-white/90 backdrop-blur-md border-2 border-dashed border-purple-200 text-center max-w-2xl mx-auto flex items-center justify-center gap-3 shadow-xs">
          <ChampionCupSvg className="w-7 h-7 shrink-0" aria-hidden="true" />
        ...
        <div className="mt-8 text-center">
          <Link
            href="/quizzes"
            className="inline-flex items-center gap-2 px-8 py-3.5 rounded-2xl bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-700 hover:to-indigo-700 text-white font-black text-xs sm:text-sm transition-all shadow-md hover:shadow-lg hover:scale-105 active:scale-[0.98]"
          >

import React from "react";
import { toast } from "sonner";
import { ChampionCupSvg } from "@/components/ui/illustrated-icons";
import { MascotItem } from "../types";

export interface MascotSelectorBarProps {
  mascots: MascotItem[];
  selectedMascot: MascotItem;
  onSelectMascot: (mascot: MascotItem) => void;
}

export const MascotSelectorBar: React.FC<MascotSelectorBarProps> = ({
  mascots,
  selectedMascot,
  onSelectMascot,
}) => {
  return (
    <div className="p-4 sm:p-6 bg-white/95 backdrop-blur-md border-2 border-purple-100 rounded-3xl shadow-md space-y-4">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2">
        <h3 className="text-xs font-black text-slate-800 flex items-center gap-2">
          <ChampionCupSvg className="w-5 h-5 shrink-0" aria-hidden="true" />
          <span>اختر تميمتك وشخصيتك البطلة:</span>
        </h3>
        <span className="text-[11px] text-purple-700 font-extrabold bg-purple-50 px-3 py-1 rounded-full border border-purple-200 shrink-0 self-start sm:self-auto">
          شخصية نشطة: {selectedMascot.name}
        </span>
      </div>

      <div 
        role="radiogroup" 
        aria-label="اختيار التميمة المرافقة" 
        className="grid grid-cols-2 sm:grid-cols-4 gap-2.5 sm:gap-3.5"
      >
        {mascots.map((m) => {
          const isSelected = selectedMascot.id === m.id;
          const IconSvg = m.SvgComponent;
          return (
            <button
              key={m.id}
              type="button"
              role="radio"
              aria-checked={isSelected}
              onClick={() => {
                onSelectMascot(m);
                toast.success(`تم اختيار ${m.name} كتميمة الطالب!`);
              }}
              className={`p-2.5 sm:p-3.5 rounded-2xl border-2 text-right transition-all flex items-center gap-2.5 sm:gap-3 cursor-pointer min-h-[44px] ${
                isSelected
                  ? "bg-purple-50/80 border-purple-500 shadow-md ring-4 ring-purple-100 scale-[1.02]"
                  : "bg-white border-purple-100 hover:border-purple-300 hover:bg-purple-50/40"
              }`}
            >
              <IconSvg className="w-8 h-8 sm:w-9 sm:h-9 shrink-0 drop-shadow-xs" aria-hidden="true" />
              <div className="min-w-0">
                <span className="text-xs font-black text-slate-900 block truncate">{m.name}</span>
                <span className="text-[10px] text-purple-600 font-bold block truncate">{m.title}</span>
              </div>
            </button>
          );
        })}
      </div>
    </div>
  );
};

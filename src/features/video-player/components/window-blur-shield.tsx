...
export function WindowBlurShield({ visible, onResume }: WindowBlurShieldProps) {
  ...
  return (
    <div
      className="absolute inset-0 bg-slate-950/95 backdrop-blur-2xl z-40 flex flex-col items-center justify-center p-6 text-center space-y-4 animate-in fade-in duration-200"
    >
      ...
      <button
        type="button"
        onClick={onResume}
        className="px-6 py-2.5 rounded-2xl bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-black shadow-lg shadow-indigo-500/25 transition-all cursor-pointer min-h-[44px]"
      >
